...
if ($this->resource->amAuthor()) {
    echo '[<i><a href="' . $this->url(array('link' => $this->resource->ident), 'links_link_edit') . '">Editar</a></i>]';
}
echo '</h1>';
...
if ($this->acl('ratings', 'new')) {
    echo '<a href="' . $this->url(array('resource' => $this->resource->ident), 'links_link_rating_down') . '"><b>&laquo;</b></a>';
}
echo '<i>' . $this->resource->ratings . ' / ' . $this->resource->raters . '</i>';
if ($this->acl('ratings', 'new')) {
echo '<a href="' . $this->url(array('resource' => $this->resource->ident), 'links_link_rating_up') . '"><b>&raquo;</b></a>';
}
echo '</td></tr>';
...
if ($this->acl('comments', 'view')) {
    ...
    echo $this->partial($this->template('comments', 'comments'), array('resource' => $this->resource, 'comment_route' => 'links_link_comment', 'config' => $this->config, 'template' => $this->template));
    if ($this->acl('comments', 'new')) {
        echo $this->partial($this->template('comments', 'comment/post'), array('resource' => $this->resource, 'comment_route' => 'links_link_comment', 'config' => $this->config, 'template' => $this->template, 'user' => $this->user));
    }
}
