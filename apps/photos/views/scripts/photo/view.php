...
if ($this->acl('comments', 'view')) {
    ...
    echo $this->partial($this->template('comments', 'comments'), array('resource' => $this->resource, 'comment_route' => 'photos_photo_comment', 'config' => $this->config, 'template' => $this->template));
    if ($this->acl('comments', 'new')) {
        echo $this->partial($this->template('comments', 'comment/post'), array('resource' => $this->resource, 'comment_route' => 'photos_photo_comment', 'config' => $this->config, 'template' => $this->template, 'user' => $this->user));
    }
}
