...
if ($this->acl('comments', 'view')) {
    ...
    echo $this->partial($this->template('comments', 'comments'), array('resource' => $this->resource, 'comment_route' => 'events_event_comment', 'config' => $this->config, 'template' => $this->template));
    if ($this->acl('comments', 'new')) {
        echo $this->partial($this->template('comments', 'comment/post'), array('resource' => $this->resource, 'comment_route' => 'events_event_comment', 'config' => $this->config, 'template' => $this->template));
    }
}
