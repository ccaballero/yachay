...
if (count($this->followers)) {
    echo '<table width="100%">';

    foreach ($this->followers as $follower) {
        $user = $this->model_users->findByIdent($follower->user);
        ...
        if ($this->acl('friends', 'contact')) {
            echo '[<a href="' . $this->url(array('user' => $user->url), 'friends_add') . '"><b><i>Agregar contacto</i></b></a>]';
        }
        ...
        echo '<td colspan="2"><b>Fecha de contacto: </b>' . $this->timestamp($follower->tsregister) . '</td></tr>';
    }

    echo '</table>';
} else {
    echo '<p>No existen peticiones registradas</p>';
}
